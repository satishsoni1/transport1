...
async function compressImage(file: File, maxWidthPx = 1200, qualityJpeg = 0.75): Promise<string> {
...
export default function DriverPodPage() {
  ...
  const handleSearch = async () => {
    const q = search.trim();
    const data = await driverFetch<DriverLR[]>(`/api/driver/lrs?search=${encodeURIComponent(q)}`);
    setLrs(data);
    const exact = data.find((item) => item.lr_no.trim().toUpperCase() === q.toUpperCase());
    if (exact) { handleSelect(exact); return; }
    if (q && data.length === 0) toast.error('No LR found for this driver');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedLR) { toast.error('Select LR first'); return; }
    if (!podImageUrl) { toast.error('Upload signed POD image'); return; }
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex gap-2">
              <Input
                ...
                placeholder="Enter LR No and press Find"
