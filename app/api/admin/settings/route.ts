      ...
      if (existingRows.length > 0) {
        ({ rows } = await sql`
          UPDATE app_settings SET
            ...
            transporter_name_font = ${body.transporter_name_font ?? existing.transporter_name_font ?? 'Arial'},
            lr_prefix = ${body.lr_prefix ?? existing.lr_prefix ?? ''},
            invoice_prefix = ${body.invoice_prefix ?? existing.invoice_prefix ?? ''},
            lr_print_format = ${body.lr_print_format ?? existing.lr_print_format ?? 'classic'},
            invoice_print_format = ${body.invoice_print_format ?? existing.invoice_print_format ?? 'classic'},
            lr_print_instructions = ${body.lr_print_instructions ?? existing.lr_print_instructions ?? ''},
            default_lr_charge = ${body.default_lr_charge === undefined ? (existing.default_lr_charge ?? 0) : Number(body.default_lr_charge) || 0},
