...
export async function PUT(
  ...
  try {
    ...
    const { rows: existingRows } = await sql`SELECT * FROM vehicles WHERE id = ${id} AND transport_id = ${transportId}`;
    ...
    const { rows: dupRows } = await sql`
      ...
      WHERE transport_id = ${transportId} AND LOWER(vehicle_no) = LOWER(${vehicleNo}) AND id <> ${id}
    `;
    if (dupRows.length > 0) {
      return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    const { rows } = await sql`DELETE FROM vehicles WHERE id = ${id} AND transport_id = ${transportId} RETURNING id`;
    if (rows.length === 0) {
      return NextResponse.json(
        { success: false, error: 'Vehicle not found' },
        { status: 404 }
